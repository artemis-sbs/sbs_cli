...
function script(rel, cam, vb, sel) {
  return V3.clientBundle() + Gizmo.clientBundle() + Nav.clientBundle()
    ...
    + 'const nm=el("fname");if(nm&&document.activeElement!==nm)nm.value=p.name||"";'
    + 'const kk=el("ikey");if(kk)kk.textContent="key: "+p.key+" (passages use it)";'
    + 'const rl=el("froles");'
    ...
    + 'const rli=el("froles");'
    + 'if(rli)rli.addEventListener("change",function(){const p=partOf(sel);if(!p)return;'
