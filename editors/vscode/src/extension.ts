...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('amd.showRelic', () => showRelic()));
...
// ---------------------------------------------------------------------------
// Relic editor - a top-down floor plan of a relic interior, drawn from the .amd
// and edited by dragging.
//
// Deliberately does NOT go through the language server. The model
// (media/relicModel.js) parses the document text directly, so the panel answers
// instantly and no rebuild is needed to see a change. The LSP is for diagnostics
// and completion; this is geometry.
//
// 2D on purpose, with height as a number rather than a third axis. A relic IS
// three-dimensional - the demo has a shaft at y=+2200 and a crypt below the
// ecliptic - but an XZ plan is what the radar already reads, and hit-testing a
// plan is a circle test rather than a picking problem.
//
...
interface RelicPart {
  ...
  hx?: number; hy?: number; hz?: number; shape?: string; nums?: number[];
}
interface RelicRec {
  key: string; name: string; loc: number[]; headingLine: number;
...
async function showRelic(uriArg?: string, column: vscode.ViewColumn = vscode.ViewColumn.Beside): Promise<void> {
  ...
  // Where the camera is pointing, remembered per PANEL rather than globally: two relic
  // files open side by side are two different things to look at.
